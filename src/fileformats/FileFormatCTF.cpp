                           ...
                           std::ostream & ostream) const
{
    ...
    if (required_lut == CTF_1D)
    {
        Lut1DOpDataRcPtr lut1D = std::make_shared<Lut1DOpData>(static_cast<unsigned long>(onedSize));
    ...
    else if (required_lut == CTF_1D_3D)
    {
        if (fromInStart != 0.f || fromInEnd != 1.0f)
        {
            CreateRangeOp(ops, double(fromInStart), double(fromInEnd), 0, 1, TRANSFORM_DIR_FORWARD);
        }
    ...
    if (required_lut == CTF_3D || required_lut == CTF_1D_3D)
    {
        Lut3DOpDataRcPtr lut3D = std::make_shared<Lut3DOpData>(static_cast<unsigned long>(cubeSize));
