...
bool hexasciitofloat(float& fval, const char * ascii)
{
    ...
    fvalbytes[0] = static_cast<unsigned char>(asciinums[1] | (asciinums[0] << 4));
    fvalbytes[1] = static_cast<unsigned char>(asciinums[3] | (asciinums[2] << 4));
    fvalbytes[2] = static_cast<unsigned char>(asciinums[5] | (asciinums[4] << 4));
    fvalbytes[3] = static_cast<unsigned char>(asciinums[7] | (asciinums[6] << 4));
#else
    // Since incoming values are little endian, and we're on big endian
    // flip the byte order
    fvalbytes[3] = static_cast<unsigned char>(asciinums[1] | (asciinums[0] << 4));
    fvalbytes[2] = static_cast<unsigned char>(asciinums[3] | (asciinums[2] << 4));
    fvalbytes[1] = static_cast<unsigned char>(asciinums[5] | (asciinums[4] << 4));
    fvalbytes[0] = static_cast<unsigned char>(asciinums[7] | (asciinums[6] << 4));
#endif
    return true;
}
...
private:
    ...
    static void StartElementHandler(void *userData,
        ...
        const XML_Char ** /*atts*/)
    {
        XMLParserHelper * pImpl = static_cast<XMLParserHelper*>(userData);
