...
{
    unsigned int dim = static_cast<unsigned int>(rawshaper.size());
                                      ...
                                      Interpolation interp) const
{
    ...
    if(!rawshaper.empty())
    {
        ...
        {
            unsigned long length = static_cast<unsigned long>(rawshaper.size());
    ...
    if(!raw3d.empty())
    {
        ...
        {
            lutArray[static_cast<unsigned long>(i)] = static_cast<float>(raw3d[i]) / scale;
        }
    }
    return cachedFile;
}
