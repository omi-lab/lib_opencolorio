...
rsr_Interpolator1D * rsr_Interpolator1D_createFromRaw( rsr_Interpolator1D_Raw * data )
{
    ...
    retval = static_cast<rsr_Interpolator1D*>(malloc( sizeof(rsr_Interpolator1D) )); // OCIO change: explicit cast
    if(retval==NULL)
    {
        return NULL;
    }

    retval->stims = static_cast<float*>(malloc( sizeof(float) * data->length )); // OCIO change: explicit cast
    ...
    retval->parameters = static_cast<float*>(malloc( 5*sizeof(float) * ( data->length - 1 ) )); // OCIO change: explicit cast
...
{
    unsigned int i;
    rsr_Interpolator1D_Raw * prelut = static_cast<rsr_Interpolator1D_Raw*>(malloc( sizeof(rsr_Interpolator1D_Raw) )); // OCIO change: explicit cast
    if(prelut==NULL) return NULL;

    prelut->stims = static_cast<float*>(malloc( sizeof(float) * prelutLength )); // OCIO change: explicit cast
    ...
    prelut->values = static_cast<float*>(malloc( sizeof(float) * prelutLength )); // OCIO change: explicit cast
                                      ...
                                      Interpolation interp) const
{
    ...
    if(useprelut[0] || useprelut[1] || useprelut[2])
    {
        ...
        for (int c = 0; c < 3; ++c)
        {
            ...
            cachedFile->prelut_from_min[c] = double(from_min);
            cachedFile->prelut_from_max[c] = double(from_max);
