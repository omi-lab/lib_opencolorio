...
bool Lut1dUtils::IMLutAlloc(IMLutStruct **plut, int num, int length)
{
    ...
    lut->tables = static_cast<unsigned short **>(malloc(4 * sizeof(unsigned short *)));
    if (lut->tables == NULL)
    {
        delete lut;
        lut = 0;
    ...
    for (i = 0; i < num; i++)
    {
        lut->tables[i] = static_cast<unsigned short *>(malloc(length * sizeof(unsigned short)));
...
)
{
    ...
    while (istream.good())
    {
        ...
        if (isdigit(*InString))
        {
            ptable[Count++] = static_cast<unsigned short>(std::stoi(InString));
            if (Count >= length)
                break;
        }
        else if (*InString != 0)
    ...
    std::string & errorLine)
{
    ...
    if (isdigit(*InString))
    {
        ...
        (lut->tables[0])[0] = static_cast<unsigned short>(std::stoi(InString));
        tablestart = 1;
    }
    else
    {
                                      ...
                                      Interpolation interp) const
{
    ...
    LocalCachedFileRcPtr cachedFile
        = LocalCachedFileRcPtr(new LocalCachedFile(inputBD,
                                                   outputBD,
                                                   static_cast<unsigned long>(lutSize),
                                                   interp));
    ...
    for (int i = 0, p = 0; i< lutSize; ++i)
    {
        for (int j = 0; j<3; ++j, ++p )
        {
            ...
            if (discreetLut1d->targetBitDepth == Lut1dUtils::IM_LUT_HALFBITS_PERCHANNEL)
            {
                ...
                half halfObj;
                halfObj.setBits(static_cast<unsigned short>(discreetLut1d->tables[srcTable][i]));
