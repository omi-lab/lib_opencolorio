...
#define L_ADJUST(val) \
  static_cast<T>((isOutInteger) ? Clamp((val)+0.5f, outMin,  outMax) : SanitizeFloat(val))
...
void BaseLut1DRenderer<inBD, outBD>::updateData(ConstLut1DOpDataRcPtr & lut)
{
  ...
  if (isLookup())
  {
    ...
    {
      static_cast<T*>(m_tmpLutR)[i] = L_ADJUST(lutValues[i*3+0] * outMax);
      static_cast<T*>(m_tmpLutG)[i] = L_ADJUST(lutValues[i*3+1] * outMax);
      static_cast<T*>(m_tmpLutB)[i] = L_ADJUST(lutValues[i*3+2] * outMax);
    }
  }
  else
  {
    ...
    for(unsigned long i=0; i<m_dim; ++i)
    {
      static_cast<float*>(m_tmpLutR)[i] = SanitizeFloat(lutValues[i*3+0] * outMax);
      static_cast<float*>(m_tmpLutG)[i] = SanitizeFloat(lutValues[i*3+1] * outMax);
      static_cast<float*>(m_tmpLutB)[i] = SanitizeFloat(lutValues[i*3+2] * outMax);
    }
  }
...
{
  delete []static_cast<T*>(m_tmpLutR); m_tmpLutR = nullptr;
  delete []static_cast<T*>(m_tmpLutG); m_tmpLutG = nullptr;
  delete []static_cast<T*>(m_tmpLutB); m_tmpLutB = nullptr;
}
...
void Lut1DRendererHalfCode<inBD, outBD>::apply(const void * inImg, void * outImg, long numPixels) const
{
  ...
  const InType * in = static_cast<const InType *>(inImg);
  OutType * out = static_cast<OutType *>(outImg);
  ...
  {
    const OutType * lutR = static_cast<const OutType *>(this->m_tmpLutR);
    const OutType * lutG = static_cast<const OutType *>(this->m_tmpLutG);
    const OutType * lutB = static_cast<const OutType *>(this->m_tmpLutB);
  ...
  {
    const float * lutR = static_cast<const float *>(this->m_tmpLutR);
    const float * lutG = static_cast<const float *>(this->m_tmpLutG);
    const float * lutB = static_cast<const float *>(this->m_tmpLutB);
...
void Lut1DRenderer<inBD, outBD>::apply(const void * inImg, void * outImg, long numPixels) const
{
  ...
  else  // Need to interpolate rather than simply lookup.
  {
    ...
    for(long i=0; i<numPixels; ++i)
    {
      ...
      out[0] = Converter<outBD>::CastValue(
            lerpf(lutR[static_cast<unsigned int>(highIdx[0])],
          lutR[static_cast<unsigned int>(lowIdx[0])],
          delta[0]));
      out[1] = Converter<outBD>::CastValue(
            lerpf(lutG[static_cast<unsigned int>(highIdx[1])],
          lutG[static_cast<unsigned int>(lowIdx[1])],
          delta[1]));
      out[2] = Converter<outBD>::CastValue(
            lerpf(lutB[static_cast<unsigned int>(highIdx[2])],
          lutB[static_cast<unsigned int>(lowIdx[2])],
          delta[2]));
      out[3] = Converter<outBD>::CastValue(in[3] * this->m_alphaScaling);
...
void Lut1DRendererHueAdjust<inBD, outBD>::apply(const void * inImg, void * outImg, long numPixels) const
{
  ...
  else  // Need to interpolate rather than simply lookup.
  {
    for(long i=0; i<numPixels; ++i)
    {
      ...
      // to save computing (1-delta) below.
      float delta[3];
      delta[0] = static_cast<float>(highIdx[0]) - idx[0];
      delta[1] = static_cast<float>(highIdx[1]) - idx[1];
      delta[2] = static_cast<float>(highIdx[2]) - idx[2];
#endif
      // Since fraction is in the domain [0, 1), interpolate using 1-fraction
      // in order to avoid cases like -/+Inf * 0. Therefore we never multiply by 0 and
      // thus handle the case where A or B is infinity and return infinity rather than
      // 0*Infinity (which is NaN).
      float RGB2[] = {
        lerpf(lutR[static_cast<unsigned int>(highIdx[0])], lutR[static_cast<unsigned int>(lowIdx[0])], delta[0]),
        lerpf(lutG[static_cast<unsigned int>(highIdx[1])], lutG[static_cast<unsigned int>(lowIdx[1])], delta[1]),
        lerpf(lutB[static_cast<unsigned int>(highIdx[2])], lutB[static_cast<unsigned int>(lowIdx[2])], delta[2])
      };
                     ...
                     const float   val)
{
  ...
  half h;
  h.setBits(static_cast<unsigned short>(totalInds));
  float base = h;
  h.setBits(static_cast<unsigned short>(totalInds + 1));
  float basePlus1 = h;
