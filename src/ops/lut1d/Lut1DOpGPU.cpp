                             ...
                             std::vector<float> & paddedChannel)
{
    ...
    const unsigned long currWidth = static_cast<unsigned long>(channel.size() / 3);
    ...
    unsigned long missingEntries = width * height - static_cast<unsigned long>(paddedChannel.size() / 3);
                            ...
                            std::vector<float> & paddedChannel) // Expects Red only.
{
    ...
    unsigned long missingEntries = width * height - static_cast<unsigned long>(paddedChannel.size());
    for (unsigned long idx = 0; idx < missingEntries; ++idx)
    {
        paddedChannel.push_back(SanitizeFloat(channel[3 * (currWidth - 1)]));
    }
