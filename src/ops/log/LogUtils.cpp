...
float GetLogSideBreak(const LogOpData::Params & params, double base)
{
  ...
  float logSideBreak = std::log2(float(params[LIN_SIDE_SLOPE] * params[LIN_SIDE_BREAK] + params[LIN_SIDE_OFFSET]));
  logSideBreak *= float(params[LOG_SIDE_SLOPE]) / std::log2(float(base));
