...
float* BaseLut3DRenderer::createOptLut(const Array::Values& lut) const
{
    ...
    float *optLut =
        static_cast<float*>(malloc(maxEntries * 3 * sizeof(float)));
...
)
{
    ...
    for (unsigned long i = 0; i < n; i++)
    {
        ...
        base_vert[i] = double(gr[base_ind + i]);
        b[i] = double(val[i]) - base_vert[i];
        y[i] = b[i];
    ...
    for (unsigned long i = 0; i < list_len; i++)
    {
        ...
        for (unsigned long j = 0; j < n; j++)
        {
            ...
            new_vert[j] = double(gr[tmp_ind + j]) - base_vert[j];
        }
        ...
        if (backsub)
        {
            ...
            if (!infeas)
            {
                ...
                {
                    x_out[path_list[tmp_ind]] = float(x2[path_order[j]] + double(x_out[path_list[tmp_ind + 1]]));
                    tmp_ind--;
                }
...
void InvLut3DRenderer::RangeTree::initialize(float *grvec, unsigned long gsz)
{
    ...
    m_depth = static_cast<unsigned long>(log2base);
...
void InvLut3DRenderer::apply(const void * inImg, void * outImg, long numPixels) const
{
    ...
    for (long i = 0; i<numPixels; ++i)
    {
        ...
        currentNumChildren[0] = static_cast<unsigned long>(levels[0].child0offsets.size());
        currentChild[0] = 0;
        currentChildInd[0] = 0;
