...
void GenericScanlineHelper<InType, OutType>::prepRGBAScanline(float** buffer, long & numPixels)
{
    ...
    *buffer = m_useDstBuffer ? reinterpret_cast<float*>(m_dstImg.m_rData + m_dstImg.m_yStrideBytes * m_yIndex)
                             : &m_rgbaFloatBuffer[0];

    if((m_inOptimizedMode&PACKED_OPTIMIZATION)==PACKED_OPTIMIZATION)
    {
        const void * inBuffer = static_cast<void*>(m_srcImg.m_rData + m_srcImg.m_yStrideBytes * m_yIndex);
...
void GenericScanlineHelper<InType, OutType>::finishRGBAScanline()
{
    ...
    {
        void * out = static_cast<void*>(m_dstImg.m_rData + m_dstImg.m_yStrideBytes * m_yIndex);

        const void * in  = m_useDstBuffer ? out : static_cast<void*>(&m_rgbaFloatBuffer[0]);
